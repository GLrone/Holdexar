...
给普通玩家用:你只需要告诉它「想盯哪些游戏」——粘贴游戏链接、从 Steam 愿望单同步、或导入一份列表,剩下的各区比价、定时刷新、降价发现与到价提醒都由它自己完成。不懂代理、爬虫、数据库也没关系,这些不需要你操心。
...
| 任务(维护面板) | 爬取队列可视化;手动触发 / 批量导入 / 收藏列表 JSON 导入;自动价格链开关 |
| 代理(维护面板) | Clash 订阅接入与节点体检账本;节点状态机(冷却 / 淘汰 / 复活 / 废弃);走线日志 |
...
3. 上手:【仪表盘】→ 粘贴游戏链接添加(或到【设置】登录 Steam 后从愿望单同步)→ 在【游戏商店】看各区价格 → 想长期盯价的游戏在卡片上点星标关注 → 到【价格提醒】设一条「降到多少通知我」

第一次打开会先弹一遍新手导览,跟着「下一步」走即可,随时能跳过;想重看就到【关于】页点应用图标。

想要更稳的自动更新(以及登录窗加载、钱包余额同步),再到【代理管理】接入自己的订阅即可——这是可选的一步,不影响你先加游戏、先看已有数据。
