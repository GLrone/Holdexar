...
2. 程序目录内 `portable.flag` —— 便携模式显式声明（U 盘 / 绿色迁移）
3. 程序目录内 `data/<slug>.db` 已存在 —— **存量布局原地继续用**（零搬迁：
   老用户升级后数据一个字节都不动）
4. 未打包（开发态）—— 机器级 `<slug>-dev` 目录（**刻意与打包态数据物理隔离**）
5. 机器级目录 `%LOCALAPPDATA%` 下的 `<slug>` —— 打包态新装默认落点

第 5 条让数据不再与 exe 同生共死：「解压到新目录覆盖升级」「直接从压缩包里
双击运行（Windows 临时解包）」「整目录删除重装」都不会再产生空白数据环境。

第 4 条解决另一半问题：**开发态过去直接写工作树内的 `data/`**，爬虫的库、
日志、备份全落在 git 仓库里，一次 `git add -A` 就会把它们收进提交（本项目的
一次真实泄漏事故正是这么发生的）。开发态落到机器级目录后，仓库工作树里
不再有任何运行时会写入的位置。第 3 条留在第 4 条之前是零搬迁的关键：
老开发检出因为仓库内已有 `data/<slug>.db` 而原地不动，第 4 条只在
「仓库内没有库、即没有东西可丢」时才生效。

机器级目录同样不可写时（极端只读环境）回退程序目录旁 `data/`，宁可退回
旧布局也不让应用起不来——数据位置不如可用性重要，且该情况会在启动告警里被点名。
"""
...
def _candidates_for(name: str) -> list[Path]:
    ...
    LOCALAPPDATA 优先于 APPDATA：漫游目录会被域环境同步，几十 MB 的
    SQLite 库放进漫游盘是事故；APPDATA / 用户主目录只作兜底。
...
def describe_layout(data_dir: Path, slug: str = APP_SLUG) -> str:
    ...
    if path == app_dir() / "data":
        ...
        # 放在其后会让未打包时这两支永远不可达、统统被标成 dev（旧实现的 bug，
        # 因为测试只在 _freeze 下断言，一直没有暴露）。
...
LAYOUT_LABEL = {
    ...
    "legacy": "存量布局（程序目录 data/，随包保留则零搬迁；升级需整目录保留）",
