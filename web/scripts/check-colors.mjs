 ...
 *
 * 补 ESLint AST 看不见的两处颜色字面量藏身处：`<style>` 块里的 CSS 与
 * 模板的行内 style（图表 option 已由 chart-option-contract 规则覆盖）。
 *
 * 判据是**字面量是否等于某个 token 的取值**：命中 → 报错并指名该用的
 * token；不在任何 token 取值里 → 刻意设计色（分类色板等），放行。
 *
 * 例外登记：`ALLOW` 清单按「文件 + 色值」登记并写明理由。
 * 用法：node scripts/check-colors.mjs [--report | --json]
 *       --report 只列不判（盘点）；不带参数则违规即 exit 1。
 */
...
/**
 * token 取值 → 定义它的 token 名（一个取值可被多个 token 共用，如 muted/dim）。
 *
 * 两类 token 不参与建表（会产出错误建议）：
 *   ① `--shadow-*`：阴影是复合值，其中的 rgba 是投影色，不是 `color:` /
 *      `background:` 该取的令牌；
 *   ② 含 `gradient(` 的复合 token：色标与基础 token 同值，基础 token 已
 *      单独建表，重复收只会让建议列表变糊。
 */
 ...
 * 不把「整文件豁免」当默认——那会让登记一处色值顺手放过整个文件，
 * 登记表就失去意义了。
 */
const ALLOW = [
  {
  ...
  {
    file: 'src/views/achievements/Index.vue',
    colors: ['#ffffff'], // 比对用归一化值（源码写 #fff，norm() 后为 #ffffff）
    reason:
      '白金卡鎏金边框的 CSS mask 镂空技巧（`linear-gradient(#fff 0 0) content-box` + ' +
      '`mask-composite: exclude`）——遮罩只取 alpha，颜色本身无语义，与 ' +
      'ProductTour.vue 的 SVG mask `fill="#fff"` 同性质（白=显示）。换令牌反而错位：' +
      '该值不参与任何可见面的着色。',
  },
  {
 ...
 *    （目前仅 chartTheme.ts 与 lib/familyColors.ts 两处）——`*` 会把文件里
 *    **该走令牌**的色值一并放过。
 *
 * ② **没有字面量需要豁免的文件不登记**。登记一个不需要豁免的文件，
 *    等于白送一份豁免。
 */

/**
 * 作用域豁免：以下选择器块内的字面量归该层自己管，不由本门禁判。
 *
 *   · 主题层（`.dark` / `:not(.dark)`）——「按主题写死」正是主题层的职责
 *     （hl-light.css 整文件是 `html:not(.dark)` 前缀，tokens.css 深色半区
 *     是 `html.dark`）。
 *   · 固定纹样层（`.hl-btn-art`）——艺术按钮整套配色是自洽的固定设计，
 *     换语义令牌会拆掉它（其中若干色值恰好等于深色 --danger 等取值）。
 *   · 固定 chrome 层（`.hl-carousel`）——圆点与箭头钉在幻灯片之内，幻灯片
 *     永远有底（调用方传 colors[i]，否则自生成渐变），白色圆点/半透明黑底
 *     箭头是压在任意底图上的恒定 chrome。
 *
 * 这几层登记**作用域**而非 ALLOW 色值清单：同文件其它处的同色值可能
 * 该走令牌，按色值放行会连它一起放过。
 *
 * ⚠️ 写前缀（`/\.hl-carousel/`），不要写 `\b`：BEM `__`/`--` 中 `_` 属 `\w`、
 * 无词边界，`\b` 会让 `__` 变体整条漏掉。
 */
 ...
 *
 * 必须按列取值而不是回一个「本行是否在作用域内」的布尔：单行写法
 * `html.dark .x { color: #fff; }` 会在**同一趟**里先 push 再 pop，
 * 行末取 stack.some() 拿到的是**外层**的状态，本行的豁免被自己弹掉——
 * 只有按列取值，单行与跨行两种写法的判定才一致。
 *
 ...
 *   ① 注释（`/* *\/`、`<!-- -->`）——注释里举例写到的色值不是在用它。
 *      不能只判行首（`^//` / `^*` / `^<!--`）：那只挡得住单行注释，
 *      跨行注释中间那些行从行首看就是普通代码，会把注释里的示例色值误报。
 ...
 *      （换上去等于把投影改成一条描边）。
 *
 * 阴影消隐必须做到**跨行**：`inset` 高光这类取值常写在跨行 box-shadow 的
 * 中间行上，只判「本行含 shadow」必漏。
 *
...
/**
 * 收 `.ts` 一起扫：色值不止住在 CSS 与模板行内 style 里，`.ts` 里的常量色板
 * 同样会「抄一份 token 值再没跟随主题」——只扫 `.vue`/`.css` 会留下「换个
 * 文件类型即可绕过」的洞。当前全库 `.ts` 色彩字面量只有 ALLOW 里两处登记
 * 在册的色板注册表，扩到 `.ts` 不产生噪声。
 */
