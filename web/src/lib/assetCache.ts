 ...
 * 重试计数与「已放弃」标记写在组件内 `ref` 里会随组件销毁归零：列表里一张
 * **永久失效**（下架包 / CDN 删档 / 旧 sub 的图被回收）的封面，每换一次筛选、
 * 每切一次板块、每重挂载一次，都要重跑一遍「裸 URL + 4 次重试」= 5 次注定
 * 失败的往返。一屏 50 张卡里只要有几张死图，来回切板块就是几十次无谓请求
 * 砸到 Steam CDN 上。
 *
 * **不写预加载器、不做 in-flight 合并。** 同一 URL 的并发图片请求浏览器自己会
 * 合并，响应也由 HTTP 缓存兜住。本模块只做浏览器**做不了**的两件事：
 * ① 把「这张已经坏了」这个判断从组件生命周期里提出来，跨实例共享；
 * ② 记住**哪一次尝试成功过**，下次直接用那个 URL——重试 URL 拼 `Date.now()`
 *    的话每次重试都是全新 URL，既绕开浏览器缓存、也绕开 CDN 边缘缓存，命中率
 *    恒为 0；换稳定后缀后，同一张图的重试在任何组件实例、任何挂载轮次里都是
 *    同一个 URL，缓存才能真正生效。
 *
 * **死图状态带 TTL 而不是永久。** CDN 抖动 / 代理闪断造成的失败不该把一张正常
 * 封面在黑名单里钉死一整个会话；窗口要长到足以让「来回切板块」这类密集重复全部
 * 命中。10 分钟是两端折中：窗口内零请求，窗口过后自动放行一次探活，素材恢复即
 * 自愈（成功路径会把它从死名单里摘掉）。
 */
