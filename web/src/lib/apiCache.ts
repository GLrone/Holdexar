 ...
 *
 * **为什么需要它，而不是靠 Pinia store**：store 是单例、状态跨路由存活，但只覆盖
 * 「已搬进 store 的那几个接口」。项目里还有一批**组件自己发的 GET**——它们的宿主
 * 面板用 `v-if` 切换（bills 三个 tab、family 十个 tab），组件一离开就被销毁，组件内的
 * `ref` 连同数据一起没了；切回来时 `onMounted` 再跑一次 `load()`，于是同一份响应被
 * 反复下载。把守卫写进每个组件（`if (!payload.value) load()`）解决不了这件事——
 * `payload` 已经随组件销毁了。
 *
 * 所以缓存放**模块作用域**：不参与响应式（这不是要驱动界面的业务状态，只是「同一份
 * 响应别下第二遍」），生命周期跟随页面会话。
 *
 * 两层复用：
