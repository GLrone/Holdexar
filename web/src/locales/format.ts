/* ════════════════════════════════════════════════════════════════════
   ...
   **统一出口**：全项目直接写死 `toLocaleString('zh-CN')` / `toLocaleTimeString('zh-CN')`
   的调用点（约 20 处）一律改走本模块——这类字面量散落在 views/ 各处，
   没有公共落点就会再次漂移回硬编码。

   **注意：zh-CN 与 en-US 的数字输出逐字相同**（分组符与小数点同形）：
   `1,234,567.891` / `1,234.50` / `1,234.57` 两种语言下完全一致。
   当前**真正**会变的只有时间：`14:30` → `02:30 PM`。
   所以这里买到的不是「修好英文界面的数字」，而是三件更长期的事：
...
export interface LocaleFormat {
   ...
   *
   * 固定位数的出口：`toFixed` 的输出在 zh-CN / en-US 下逐字相同（见文件头），
   * 而 `round2` 会改变显示（`3.0` 塌成 `3`）——两者语义不同，不可互相替代。
   * 本出口让「统一走出口」与「不改显示」两个要求不冲突；
