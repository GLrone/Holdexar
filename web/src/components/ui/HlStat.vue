 ...
 *
 * 起因：同一个 family 模块里 5 个 tab 各写了一套 KPI 卡样式，5 个类名前缀
 * （`fx-mini-stat` / `lib-stat` / `pa-kpi` / `vi-kpi` / `wl-kpi`），字号 18/19/22/24、
 * 圆角 3 种、对齐 2 种、padding 4 种；`.fx-mini-stat` 一个类名还有四份定义，
 * 其中两份左对齐两份居中。这是「没有规范」最直接的证据，也是本次收敛的靶子。
 *
 * 收敛后的取值（不再逐处挑）：
 *   sm  数字 18px / padding 8×6  / radius-sm —— 6~7 列的密集行（家庭库、愿望单）
 *   md  数字 22px / padding 11×10/ radius    —— 4~6 列的常规行（family 各 tab、工具箱）
 *   lg  数字 26px / padding 18   / radius-lg —— 主视觉大数（原 `.hl-stat`）
 *
 * **默认居中**：KPI 卡是一排并列的数字，居中读数最快；左对齐仅用于卡内含长文本的
 * 场景（`align="left"`）。原先把「该居中的写成了左对齐」当默认，正是用户反馈的
 * 「很多地方不知道中间对齐」。
 *
