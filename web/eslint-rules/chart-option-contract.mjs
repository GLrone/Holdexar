 ...
 *
 * 为什么是自写规则、而不是再加一条 no-restricted-syntax 选择器：
 * 上一版唯一被机器强制的图表约束是 TOOLTIP_THEME_SELECTOR
 *   Property[key.name=/^tooltip$/] > ObjectExpression:not(:has(Property[key.name=/^backgroundColor$/]))
 * 它有两个结构性漏洞：
 *   ① 只断言「存在」——`backgroundColor: '#fff'` 照样过（白底白字，主题化形同虚设）；
 *   ② `> ObjectExpression` 要求值是**直接子级对象字面量**——写成 `tooltip: tt` 整条规则
 *      静默失配，等于没有护栏。
 * 于是 rates / bills 两张图的轴色、系列色、animation 全写死也照样过了四层门禁进仓库。
 * 这里改成按「option 对象」（含 series 属性的对象字面量）定位，逐项做实质断言。
 *
 * 四条断言：
 ...
 *                       的灰线 + 灰底白字标签框，两个主题下都突兀。
 *   3. tooltip 主题     必须对象字面量 + 显式 backgroundColor。值是变量引用同样报错——
 *                       「无法静态校验」在这条红线上等于「没写」。
 *   4. 无颜色字面量     option 子树内禁止 #hex / rgb() / rgba() / hsl()。写死的色值
 *                       不会跟随主题切换（历史事故：#8f98a0 是深色主题 --text-muted 的
 *                       取值，被抄成字面量后主题一切换就再不跟随）。
 *                       用 token 而非 AST 递归扫描：token 覆盖整段区间，且能看见**模板
 *                       字面量里内嵌的颜色**——`<span style="color:#2ed573">` 正是漏网的那处。
 */
const COLOR_RE =
...
export default {
  meta: {
    ...
    messages: {
      ...
      needAxisPointer:
        '图表红线：option 有坐标轴时必须显式声明 axisPointer（走 api/chartTheme.ts 的 axisPointerStyle 出口）。悬停时贴在轴上的指示线与轴标签框不配就是 echarts 默认的灰线 + 灰底白字，两个主题下都突兀——tooltip 主题化覆盖不到它。',
      needTooltipTheme:
      ...
      noColorLiteral:
        '图表红线：option 内禁止颜色字面量（#hex / rgb() / rgba() / hsl()），一律从 api/chartTheme.ts 的 useChartPalette() / useTipPalette() 取 CSS token。写死的色值不跟随主题切换——历史事故：#8f98a0 是深色主题 --text-muted 的取值，被抄成字面量后再切主题就不跟随了。',
    },
  },
  create(context) {
